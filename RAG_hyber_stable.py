...
def setup_models():
    ...
    reranker = FlagAutoReranker.from_finetuned(local_path, query_max_length=256, passage_max_length=512, use_fp16=True, devices=['cuda:1'])
...
def advanced_rag(query, top_K, with_graph=True):
    ...
    if with_graph==True:
        retrieved_graph = graph_retriever.get_relevant_documents(query,k=20)
    else:
        retrieved_graph = []
...
csv_file_path = "response_hyper.csv"
