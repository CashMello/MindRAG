...
def advanced_rag(query, top_K):
    ...
    print(reranked_docs)
    response = generation_chain.invoke({"question": query, "context": reranked_docs})
...
responses = []
uniq_data = dict(zip(querys[:1000],answers[:1000]))
for query, answer in uniq_data.items():
    response = advanced_rag(query,top_K)
    responses.append({
        "query":query,
        "response":response,
        "answer":answer})
